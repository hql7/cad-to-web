      ...
      console.log('进入提交程序')
        ...
        if ( validation == 'valid' ) {
            ...
            readStream.on("end", function() {
                ...
                fs.exists(chunkFilename, function (exists) {
                    console.log(chunkFilename, 11, exists);  //false
                   }); 
                var currentTestChunk =1 ;
                var numberOfChunks =Math.max (Math.floor (totalSize / (chunkSize * 1.0)), 1) ;
                console.log("numberOfChunks:" + numberOfChunks);
                var testChunkExists =function () {
                    console.log('currentTestChunk---', getChunkFilename (currentTestChunk, identifier));
                    fs.exists (getChunkFilename (currentTestChunk, identifier), function (exists) {
                        console.log('rrrrrr---', exists);  //false
                        if ( exists ) {
                            currentTestChunk++ ;
                            console.log(currentTestChunk,'--',numberOfChunks);
                            console.log("currentTestChunk > numberOfChunks:" + currentTestChunk > numberOfChunks);
                            if ( currentTestChunk > numberOfChunks )
                                callback ('done', filename, original_filename, identifier, totalSize);
                            else // Recursion
                                testChunkExists () ;
                        } else {
                            //console.log('6666---', filename);
                            callback ('partly_done', filename, original_filename, identifier, totalSize) ;
                        }
                    }) ;
                } ;
